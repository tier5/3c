...
export class TwilioSearchPipe implements PipeTransform {
  transform(items: any[], term: any): any[] {
    ...
    return items.filter(function (el: any) {
      ...
      const name = el.user_info.first_name + ' ' + el.user_info.last_name;
      const userType = el.user_info.type == '1' ? 'Main Account' : 'Subaccount';
      if (name.toLowerCase().indexOf(termLower) > -1) {
        return name.toLowerCase().indexOf(termLower) > -1;
      } else {
        // check for last name
        if (el.user_info.email.toLowerCase().indexOf(termLower) > -1) {
          return el.user_info.email.toLowerCase().indexOf(termLower) > -1;
        } else {
          // search for company
          if (el.twilio_sid.toLowerCase().indexOf(termLower) > -1) {
            return el.twilio_sid.toLowerCase().indexOf(termLower) > -1;
          } else {
            // search for email
            if (userType.toLowerCase().indexOf(termLower) > -1) {
